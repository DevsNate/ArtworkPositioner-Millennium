...
const BIG_PICTURE_TARGET_ATTRIBUTE = 'data-artwork-positioner-bigpicture-hero-appid';
...
const buildHeroPositionCss = (offsets: HeroOffsets, mode: HeroMode) => {
  ...
  const rules = Array.from(appids).map((appid) => {
    ...
    const selectors = mode === 'bigpicture'
      ? [`[${BIG_PICTURE_TARGET_ATTRIBUTE}="${appid}"]`]
      : heroImageSelectorsForApp(appid);
...
let bigPictureArtworkClasses: { ImgContainer?: string; ImgSrc?: string } | null = null;

const resolveBigPictureDetailHeroClasses = () => {
  if (!bigPictureArtworkClasses) {
    try {
      bigPictureArtworkClasses = findModule((module: any) =>
        Boolean(module?.TopCapsule && module?.ImgContainer && module?.ImgSrc),
      ) as { ImgContainer?: string; ImgSrc?: string } | null;
    } catch {
      bigPictureArtworkClasses = null;
    }
  }
  return [bigPictureArtworkClasses?.ImgContainer, bigPictureArtworkClasses?.ImgSrc]
    .filter((className): className is string => Boolean(className));
};

const markBigPictureDetailHero = (offsets: HeroOffsets, targetDocument: Document) => {
  const detailHeroClasses = resolveBigPictureDetailHeroClasses();
  const activeImages = new Set<HTMLImageElement>();
  if (detailHeroClasses.length > 0) {
    const appids = new Set<string>();
    Object.keys(offsets).forEach((key) => {
      const parsedKey = parseOffsetKey(key);
      if (parsedKey?.mode !== 'bigpicture') return;
      const horizontal = clampOffset(offsetForMode(offsets, parsedKey.appid, 'bigpicture', 'horizontal'));
      const vertical = clampOffset(offsetForMode(offsets, parsedKey.appid, 'bigpicture', 'vertical'));
      if (horizontal !== DEFAULT_OFFSET || vertical !== DEFAULT_OFFSET) appids.add(parsedKey.appid);
    });

    for (const image of targetDocument.querySelectorAll<HTMLImageElement>('img')) {
      if (!detailHeroClasses.some((className) => image.classList.contains(className))) continue;
      const appid = Array.from(appids).find((candidate) =>
        image.matches(heroImageSelectorsForApp(candidate).join(',')),
      );
      if (!appid) continue;
      image.setAttribute(BIG_PICTURE_TARGET_ATTRIBUTE, appid);
      activeImages.add(image);
    }
  }

  for (const image of targetDocument.querySelectorAll<HTMLImageElement>(`[${BIG_PICTURE_TARGET_ATTRIBUTE}]`)) {
    if (!activeImages.has(image)) image.removeAttribute(BIG_PICTURE_TARGET_ATTRIBUTE);
  }
};
...
const applyHeroPositionStyles = (offsets: HeroOffsets, targetDocument: Document | null, mode: HeroMode) => {
  ...
  markBigPictureDetailHero(offsets, targetDocument);
...
const scheduleDocumentReconcile = (targetDocument: Document, mode: HeroMode) => {
  ...
  (targetDocument.defaultView ?? window).queueMicrotask(() => {
    ...
    applyHeroPositionStyles(readOffsets(), targetDocument, mode);
    reconcileHeroPositionMenu(targetDocument);
  });
};

const observeArtworkDocument = (targetDocument: Document, mode: HeroMode) => {
  if (menuDocumentSubscriptions.has(targetDocument)) {
    scheduleDocumentReconcile(targetDocument, mode);
    return;
  }

  menuDocumentSubscriptions.set(
    targetDocument,
    subscribeToDocument(targetDocument, () => scheduleDocumentReconcile(targetDocument, mode)),
  );
};
...
const ensureBigPictureIntegration = () => {
  ...
  if (bigPictureDocument) {
    observeArtworkDocument(bigPictureDocument, 'bigpicture');
  }
  if (desktopDocument) observeArtworkDocument(desktopDocument, 'desktop');
};
...
export const stopHeroModule = () => {
  ...
  injectedStyleDocuments.forEach((targetDocument) => {
    ...
    targetDocument.querySelectorAll<HTMLImageElement>(`[${BIG_PICTURE_TARGET_ATTRIBUTE}]`)
      .forEach((image) => image.removeAttribute(BIG_PICTURE_TARGET_ATTRIBUTE));
  });
